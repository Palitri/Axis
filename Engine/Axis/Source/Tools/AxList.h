...
template <typename T> class AxList
{
public:
	...
	void Move(int originalIndex, int newIndex, int count)
	{
		...
		T *temp = new T[count];
		AxMem::Copy(temp, &this->values[originalIndex], count * sizeof(T));
		...
		AxMem::Copy(&this->values[newIndex], temp, count * sizeof(T));
		delete[] temp;
	};
	...
	int IndexOf(T value)
	{
		for (int i = 0; i < this->count; i++)
			if (AxMem::Compare(&this->values[i], &value, sizeof(T)) == 0)
				return i;
