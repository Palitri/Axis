...
template <typename TKey, typename TValue> struct AxDictionaryEntry
{
	TKey key;
	TValue value;
};
...
template <typename TKey, typename TValue> class AxDictionary
	: public AxLinkedList<AxDictionaryEntry<TKey, TValue>>
{
private:
	bool FindEntryNode(TKey key, void *&node)
	{
		...
		{
			AxDictionaryEntry<TKey, TValue> entry = this->GetNodeValue(node);
			if (entry.key == key)
				return true;
...
public:
	AxDictionary(void)
		: AxLinkedList<AxDictionaryEntry<TKey, TValue>>()
	{
	};

	virtual ~AxDictionary(void)
	{
	};
	...
	bool Add(TKey key, TValue value)
	{
		...
		AxDictionaryEntry<TKey, TValue> entry;
		entry.key = key;
		entry.value = value;
		if (nextBiggest != 0)
