...
Soras::Soras(void)
{
	...
	this->SetThreadCount(AxPlatformUtils::GetNumberOfProcessors());
}
...
bool Soras::NormalizationNecessary()
{
    ...
    float l = AxMath::Sqrt(this->worldMatrixNormals._11 * this->worldMatrixNormals._11 +
						  this->worldMatrixNormals._12 * this->worldMatrixNormals._12 +
						  this->worldMatrixNormals._13 * this->worldMatrixNormals._13);
    if (AxMath::Abs(1.0f - l) > necessityThreshold)
        return true;

    l = AxMath::Sqrt(this->worldMatrixNormals._21 * this->worldMatrixNormals._21 +
					this->worldMatrixNormals._22 * this->worldMatrixNormals._22 +
					this->worldMatrixNormals._23 * this->worldMatrixNormals._23);
    if (AxMath::Abs(1.0f - l) > necessityThreshold)
        return true;

	l = AxMath::Sqrt(this->worldMatrixNormals._31 * this->worldMatrixNormals._31 +
...
void Soras::ClearZ()
{
	AxMem::Copy(this->zBuffer, this->zBufferStencil, this->renderSurface->width * this->renderSurface->height * sizeof(float));
}
...
void Soras::SetThreadCount(int threadCount)
{
	...
	if (threadCount != 0)
		this->threadsPool = new AxThreadsPool(threadCount);
}
	...
	if (renderSurface != 0)
	{
		...
		if (newZBufferSize != this->zBufferSize)
		{
			...
			for (unsigned int i = 0; i < this->zBufferSize; i++)
				this->zBufferStencil[i] = AxMath::FloatMax;
		}
	}
...
void Soras::SetLighsCount(int numLights)
{
	if (numLights > this->maxLights)
	{
		...
		if (this->maxLights > 0)
		{
			AxMem::Copy(newLights, this->lights, this->maxLights * sizeof(SrLight));
			delete[] this->lights;
		}
...
void Soras::RenderMesh()
{
	...
	AxIntegralQuotientDistribution distribution(this->mesh->numVertices, this->renderThreads.count);
